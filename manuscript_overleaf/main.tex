% Springer Nature Overleaf-ready final package generated from the project manuscript.
% Template source: Springer Nature sn-jnl class.
...
\title[Self-supervised CTG phenotyping]{Self-supervised dynamic cardiotocography phenotyping for calibrated neonatal risk enrichment: an open-data decision-modelling study}
...
\abstract{\textbf{Background:} Intrapartum cardiotocography (CTG) records fetal heart rate (FHR) and uterine contraction (UC) dynamics, but decision support often relies on static summaries or subjective pattern recognition. We evaluated whether self-supervised CTG representations could support calibrated neonatal risk enrichment while preserving interpretability and explicit validation boundaries.
...
\textbf{Conclusions:} Open-data CTG self-supervised phenotyping can support interpretable, calibration-aware neonatal risk enrichment. The current study does not establish external outcome validity or clinical deployability.}

\keywords{cardiotocography, fetal heart rate, self-supervised learning, PatchTST, dynamic phenotyping, calibration, neonatal risk}
...
The primary CTU-UHB/CTU-CHB dataset is publicly available through PhysioNet \cite{goldberger2000,chudacek2014,ctuuhb}. CTGDL-FHRMA is available through its public Zenodo record with subset-specific access caveats \cite{ctgdl}. UCI Cardiotocography is available through the UCI Machine Learning Repository \cite{uci}. Derived source-data tables for each figure and supplementary CSV tables are included in the manuscript package and should be deposited with the submission or archived in a persistent repository before journal submission.

\section{Code availability}

The local analysis pipeline is organized under \path{scripts/}. The transformer SSL encoder was trained by \path{scripts/04_models/train_transformer_ssl_encoder.py}; downstream embeddings, phenotypes, validation, recalibration, reviewer-requested sensitivity analyses and figure source-data tables were generated by the remaining scripts in \path{scripts/04_models/} and \path{scripts/05_reports/}. Manuscript figures were generated by \path{scripts/05_reports/build_manuscript_figures.py}. Figure legends, Results text, Methods text, and supplementary tables were generated by \path{scripts/05_reports/build_manuscript_text_and_supplement.py}. The submission package includes figure source-data tables, supplementary CSV tables, the fixed record-level split and the sensitivity-analysis table. A public code archive with scripts, environment files, fixed random seeds, split files and generated source-data tables must be created before journal submission.
...
\begin{figure}[H]
  ...
  \caption{\textbf{Self-supervised CTG representations improve risk enrichment and contribute measurable AI features.}
